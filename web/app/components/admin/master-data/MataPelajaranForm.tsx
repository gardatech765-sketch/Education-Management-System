"use client";

import { useState } from "react";
import Toggle from "@/components/admin/ui/Toggle";
import { JENJANG_OPTIONS, type Jenjang, type MataPelajaran } from "./dummy";

export type MataPelajaranFormValues = {
  nama_mapel: string;
  jenjang: Jenjang;
  deskripsi: string;
  is_active: boolean;
};

const EMPTY: MataPelajaranFormValues = {
  nama_mapel: "",
  jenjang: "SMA",
  deskripsi: "",
  is_active: true,
};

function toFormValues(initial?: MataPelajaran | null): MataPelajaranFormValues {
  return initial
    ? {
        nama_mapel: initial.nama_mapel,
        jenjang: initial.jenjang,
        deskripsi: initial.deskripsi,
        is_active: initial.is_active,
      }
    : EMPTY;
}

export default function MataPelajaranForm({
  initial,
  onSubmit,
  formId,
}: {
  initial?: MataPelajaran | null;
  onSubmit: (values: MataPelajaranFormValues) => void;
  formId: string;
}) {
  const [values, setValues] = useState<MataPelajaranFormValues>(() => toFormValues(initial));
  const [error, setError] = useState<string | null>(null);


  function handleSubmit(e: React.FormEvent) {
    e.preventDefault();
    if (!values.nama_mapel.trim()) {
      setError("Nama mata pelajaran wajib diisi.");
      return;
    }
    onSubmit(values);
  }

  return (
    <form id={formId} onSubmit={handleSubmit} className="space-y-4">
      {error && (
        <p className="rounded-lg bg-red-50 px-3 py-2 text-sm text-red-700">{error}</p>
      )}

      <div>
        <label className="mb-1.5 block text-sm font-medium text-zinc-700">
          Nama Mata Pelajaran
        </label>
        <input
          type="text"
          value={values.nama_mapel}
          onChange={(e) => setValues((v) => ({ ...v, nama_mapel: e.target.value }))}
          placeholder="cth. Matematika"
          className="w-full rounded-lg border border-zinc-200 px-3 py-2 text-sm outline-none focus:border-amber-400 focus:ring-2 focus:ring-amber-100"
        />
      </div>

      <div>
        <label className="mb-1.5 block text-sm font-medium text-zinc-700">Jenjang</label>
        <select
          value={values.jenjang}
          onChange={(e) => setValues((v) => ({ ...v, jenjang: e.target.value as Jenjang }))}
          className="w-full rounded-lg border border-zinc-200 px-3 py-2 text-sm outline-none focus:border-amber-400 focus:ring-2 focus:ring-amber-100"
        >
          {JENJANG_OPTIONS.map((j) => (
            <option key={j} value={j}>
              {j}
            </option>
          ))}
        </select>
      </div>

      <div>
        <label className="mb-1.5 block text-sm font-medium text-zinc-700">Deskripsi</label>
        <textarea
          value={values.deskripsi}
          onChange={(e) => setValues((v) => ({ ...v, deskripsi: e.target.value }))}
          rows={3}
          placeholder="Deskripsi singkat mata pelajaran"
          className="w-full resize-none rounded-lg border border-zinc-200 px-3 py-2 text-sm outline-none focus:border-amber-400 focus:ring-2 focus:ring-amber-100"
        />
      </div>

      <div className="flex items-center justify-between rounded-lg border border-zinc-200 px-3 py-2.5">
        <span className="text-sm font-medium text-zinc-700">Status Aktif</span>
        <Toggle
          checked={values.is_active}
          onChange={(v) => setValues((prev) => ({ ...prev, is_active: v }))}
        />
      </div>
    </form>
  );
}